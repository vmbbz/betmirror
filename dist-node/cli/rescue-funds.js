...
// --- File Scanner ---
function scanLocalFilesForKey(targetAddress) {
...
async function run() {
    ...
    if (balance <= 0n) {
        ...
        if (signerBal > 0n) {
            ...
            if (q.toLowerCase() === 'y') {
                ...
                const tx = await usdcContract.connect(signer).transfer(signer.address, signerBal);
    ...
    const txHashBytes = await safeContract.getTransactionHash(TOKENS.USDC_BRIDGED, 0, data, 0, 0, 0, 0, "0x0000000000000000000000000000000000000000", "0x0000000000000000000000000000000000000000", nonce);
