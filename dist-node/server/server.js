...
app.post('/api/wallet/activate', async (req, res) => {
    ...
    try {
        ...
        const configToSave = { ...walletConfig, type: 'GNOSIS_SAFE', safeAddress: safeAddr, isSafeDeployed: false };
...
async function seedRegistry() {
    ...
    for (const address of systemWallets) {
        ...
        try {
            ...
            if (!exists) {
                await Registry.create({ address: normalized, listedBy: 'SYSTEM', listedAt: new Date().toISOString(), isSystem: true, tags: ['OFFICIAL', 'WHALE'], winRate: 0, totalPnl: 0, tradesLast30d: 0, followers: 0, copyCount: 0, copyProfitGenerated: 0 });
            }
