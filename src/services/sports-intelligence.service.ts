...
export class SportsIntelligenceService extends EventEmitter {
    ...
    private readonly sportsWsUrl = 'wss://sports-api.polymarket.com/ws';
    private nexusMapping: Map<number, any[]> = new Map(); // gameId -> Token Metadata
     ...
     */
    private async refreshMatchMappings(): Promise<void> {
        try {
            ...
            for (const sport of sports) {
                ...
                if (!Array.isArray(eventsRes.data)) continue;

                for (const event of eventsRes.data) {
                    const gameId = event.gameId; // Unique Number linking news to markets
                    if (!gameId || !event.markets || event.markets.length === 0) continue;
